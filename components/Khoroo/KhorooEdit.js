...
const Page = ({ params: { slug } }) => {
  ...
  const { updateKhoroo, getKhoroo, khoroo } = useKhoroo();
  ...
  useEffect(() => {
    if (khoroo) {
      if (khoroo.polygon && khoroo.polygon.properties) {
        const polygonData = {
          value: khoroo.polygon._id,
          label: khoroo.polygon.properties.name,
        };
      ...
      if (khoroo.cityProvince) {
        const cdata = {
          value: khoroo.cityProvince._id,
          label: khoroo.cityProvince.name,
        };

        handleSelectCity(khoroo.cityProvince._id);
        setSelectCity(khoroo.cityProvince._id);
        setIsDistrict(false);
      ...
      if (khoroo.district) {
        const cdata = {
          value: khoroo.district._id,
          label: khoroo.district.name,
        };

        const isDuplicate = districtData.some(
          (data) => data.value === cdata.value
        );
        if (!isDuplicate) setDistrictData((prevDatas) => [cdata, ...prevDatas]);
      }

      form.setFieldsValue({
        ...khoroo,
        polygon: khoroo.polygon && khoroo.polygon._id,
        cityProvince: khoroo.cityProvince && khoroo.cityProvince._id,
        district: khoroo.district && khoroo.district._id,
      });
    }
  }, [khoroo]);
  ...
  const handleAdd = async (values) => {
    ...
    const result = await updateKhoroo(sendData);
