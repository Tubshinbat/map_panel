...
const Page = ({ params: { slug } }) => {
  ...
  const { updateBankAccount, getBankAccount, singleBankAccount } =
    useBankAccounts();
  ...
  useEffect(() => {
    if (singleBankAccount) {
      ...
      setCheckedRadio(() => ({
        star: singleBankAccount.star,
      }));

      if (singleBankAccount.bankLogo)
        setPhoto({
          name: singleBankAccount.bankLogo,
          url: `${base.cdnUrl}/150x150/${singleBankAccount.bankLogo}`,
        });
    }
  }, [singleBankAccount]);
  ...
  const handleEdit = async (values) => {
    ...
    const result = await updateBankAccount(sendData, slug);
    if (result)
      setTimeout(() => {
