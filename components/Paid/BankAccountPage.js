...
const Page = () => {
  ...
  const { bankAccounts, pagination, loadBankAccount, deleteBankAccount } =
    useBankAccounts();
      ...
      render: (text, record) => {
        return (
          <div className="table-image">
            {record.bankLogo ? (
              <img src={`${base.cdnUrl}/150x150/${record.bankLogo}`} />
            ) : (
  ...
  const handleEdit = () => {
    ...
    else router.push(`/paid_type/edit/${selectedRowKeys[0]}`);
  };
    ...
    switch (modal) {
      ...
      case "edit": {
        if (selectedRowKeys && selectedRowKeys.length === 1) {
          router.push("/paid_type/edit/" + selectedRowKeys[0]);
        } else {
          toastControl("error", "Нэг өгөгдөл сонгоно уу");
        }
        break;
  ...
  const handleDelete = async () => {
    await deleteBankAccount(selectedRowKeys);
    handleCancel();
    setSelectedRowKeys([]);
  };
        ...
        <div className="card card-body py-5 px-4">
          <div className="datatable-header-tools">
            <div className="datatable-actions">
              <Button
                onClick={() => router.push(`/paid_type/add`)}
