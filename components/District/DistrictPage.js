...
const Page = () => {
  ...
  const { districts, pagination, loadDistrict, deleteDistrict } = useDistrict();
  ...
  const handleEdit = () => {
    ...
    else router.push(`/district/edit/${selectedRowKeys[0]}`);
  };
    ...
    switch (modal) {
      ...
      case "edit": {
        if (selectedRowKeys && selectedRowKeys.length === 1) {
          router.push("/district/edit/" + selectedRowKeys[0]);
        } else {
          toastControl("error", "Нэг өгөгдөл сонгоно уу");
        }
        break;
  ...
  const handleDelete = async () => {
    await deleteDistrict(selectedRowKeys);
    handleCancel();
    setSelectedRowKeys([]);
  };
          ...
          <div className="card card-body py-5 px-4">
            <div className="datatable-header-tools">
              <div className="datatable-actions">
                <Button
                  onClick={() => router.push(`/district/add`)}
