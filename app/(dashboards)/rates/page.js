...
const Page = () => {
  ...
  const { rates, pagination, loadRate, deleteRate } = useRate();
  ...
  const handleEdit = () => {
    ...
    else router.push(`/rates/edit/${selectedRowKeys[0]}`);
  };
    ...
    switch (modal) {
      ...
      case "edit": {
        if (selectedRowKeys && selectedRowKeys.length === 1) {
          router.push("/rates/edit/" + selectedRowKeys[0]);
        } else {
          toastControl("error", "Нэг өгөгдөл сонгоно уу");
        }
        break;
  ...
  const handleDelete = async () => {
    await deleteRate(selectedRowKeys);
    handleCancel();
    setSelectedRowKeys([]);
  };
